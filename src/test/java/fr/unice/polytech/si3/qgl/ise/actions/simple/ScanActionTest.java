...
public class ScanActionTest {
    ...
    public void testProbability() {
        ...
        assertEquals(IslandMap.getPercentageOfLayerForUpdate()[0],
                map.getTile(new Coordinates(0, 0)).getBiomesPercentage().get(Biome.GLACIER), 0.01);

        assertEquals(IslandMap.getPercentageOfLayerForUpdate()[1],
                map.getTile(new Coordinates(2, 0)).getBiomesPercentage().get(Biome.GLACIER), 0.01);

        assertEquals(IslandMap.getPercentageOfLayerForUpdate()[2],
                map.getTile(new Coordinates(3, 0)).getBiomesPercentage().get(Biome.GLACIER), 0.01);

        assertEquals(IslandMap.getPercentageOfLayerForUpdate()[3],
                map.getTile(new Coordinates(4, 0)).getBiomesPercentage().get(Biome.GLACIER), 0.01);

        assertEquals(IslandMap.getPercentageOfLayerForUpdate()[4],
                map.getTile(new Coordinates(3, 3)).getBiomesPercentage().get(Biome.GLACIER), 0.01);

        assertEquals(IslandMap.getPercentageOfLayerForUpdate()[5],
                map.getTile(new Coordinates(3, 4)).getBiomesPercentage().get(Biome.GLACIER), 0.01);

        assertEquals(IslandMap.getPercentageOfLayerForUpdate()[6],
                map.getTile(new Coordinates(4, 4)).getBiomesPercentage().get(Biome.GLACIER), 0.01);
        ...
        assertEquals(map.getTile(new Coordinates(0, 0)).getBiomesPercentage().get(Biome.ALPINE),
                100, 0.01);

        double percentage = IslandMap.getPercentageOfLayerForUpdate()[1];
        assertEquals(percentage + (((100d - percentage) * percentage) / 100),
                map.getTile(new Coordinates(2, 0)).getBiomesPercentage().get(Biome.ALPINE), 0.01);

        percentage = IslandMap.getPercentageOfLayerForUpdate()[2];
        assertEquals(percentage + (((100d - percentage) * percentage) / 100),
                map.getTile(new Coordinates(3, 0)).getBiomesPercentage().get(Biome.ALPINE), 0.01);

        percentage = IslandMap.getPercentageOfLayerForUpdate()[3];
        assertEquals(percentage + (((100d - percentage) * percentage) / 100),
                map.getTile(new Coordinates(4, 0)).getBiomesPercentage().get(Biome.ALPINE), 0.01);

        percentage = IslandMap.getPercentageOfLayerForUpdate()[4];
        assertEquals(percentage + (((100d - percentage) * percentage) / 100),
                map.getTile(new Coordinates(3, 3)).getBiomesPercentage().get(Biome.ALPINE), 0.01);

        percentage = IslandMap.getPercentageOfLayerForUpdate()[5];
        assertEquals(percentage + (((100d - percentage) * percentage) / 100),
                map.getTile(new Coordinates(3, 4)).getBiomesPercentage().get(Biome.ALPINE), 0.01);
        ...
        for (int i = 0; i < 20; ++i)
            drone.acknowledgeScan(new Scan("{\"cost\": 2, \"extras\": { \"biomes\": [\"GLACIER\", \"ALPINE\"], " +
                    "\"creeks\": [], \"sites\": []}, \"status\": \"OK\"}"));
        assertTrue(map.getTile(new Coordinates(2, 0)).getBiomesPercentage().get(Biome.GLACIER) < 100);
